...
$pasta_img = $_FILES["file"]["tmp_name"];
$arquivo = $_FILES["file"]["name"];
$pasta = "assets/images";

move_uploaded_file($pasta_img, $pasta . "/" . $arquivo);

$file = fopen('assets/images/vendas.txt', 'r');

while (!fopen($file)){
    $data = explode(';', fgets($file)); // separar valores pelos ; que estão no ficheiro
    $query = mysqli_query($conecta, 'INSERT INTO vendas (DESCRICAO, PRECO, QTD, CLIENTE_NOME, CLIENTE_CPF_CNPJ, CLIENTE_ENDERECO) VALUES (' .implode(', ', $data). ')');
}
                                ...
                                <form method="post" enctype="multipart/form-data">
                                        <div class="row mb-5">
                                            <div class="col-lg-4 col-sm-12 mb-4 mb-lg-0">
                                                <input type="file" name="file" accept=".txt, .jpg, .png, image/jpeg, image/png"  class="dropify" data-bs-height="180" />
                                            </div>
                                            <button type="submit" class="btn btn-primary btn-pill" name="update">Enviar</button>
                                        </div>
</form>
</div>
                            </div>
